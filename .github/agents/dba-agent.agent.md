...
- **Database Engine:** SQL Server 2022 (pymssql) and PostgreSQL 16 (psycopg2-binary)
- **ORM / Query Builder:** Raw SQL with dialect abstraction layer (`app/scenarios/dialect.py`)
- **Migration Tool:** Custom deploy script (`app/sample_db/deploy.py`) — drops and recreates tables
- **Migration Command:** POST `/api/deploy-sample-db` (via UI or API)
- **Database Connection:** Configured at runtime via UI or POST `/api/connect` with host, port, database, username, password
