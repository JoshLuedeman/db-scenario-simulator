...
- **Cloud Provider:** N/A (runs locally in containers)
- **IaC Tool:** N/A
- **Container Runtime:** Docker or Apple Containers (auto-detected by `run.sh`)
- **Orchestration:** Docker Compose with profiles (sqlserver, postgres, full)
- **Build Command:** `docker build -f Containerfile -t db-scenario-simulator .`
- **Deploy Command:** `./run.sh full` or `docker compose --profile full up --build`
